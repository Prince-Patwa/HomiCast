    ...
    try:
        if req.session['ownerid']!=None:
            ...
            if req.method=="POST":
                ...
                addsell=AddSell(image=image,price=price,apartment=apartment,area=area,location=location,pimages=pimages,add_date=add_date,emailId=owner.emailaddress,name=owner.name)
                addsell.save()
                return redirect('renter_buyer_app:viewsell')
...
def viewsell(req):
    return render(req,'viewsell.html')
